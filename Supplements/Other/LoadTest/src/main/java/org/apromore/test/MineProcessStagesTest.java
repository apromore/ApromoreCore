...
public class MineProcessStagesTest extends org.apromore.test.Test {

	public MineProcessStagesTest()
	{
	...
	{					
		...
		List<MineProcessStage> pluginList = new ArrayList<MineProcessStage>(testSetting.nConcurrentUsers);
		for(int i = 0; i < testSetting.nConcurrentUsers; i++)
		{
			try {
				pluginList.add(new MineProcessStage(testSetting));
			}catch(Exception ex)
			{
