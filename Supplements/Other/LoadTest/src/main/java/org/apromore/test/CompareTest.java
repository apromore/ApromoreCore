...
public class CompareTest extends org.apromore.test.Test {

	public CompareTest()
	{
		super.init("CompareFolder", 
				getClass().getName(),				
	...
	{					
		...
		List<Compare> pluginList = new ArrayList<Compare>(testSetting.nConcurrentUsers);
		for(int i = 0; i < testSetting.nConcurrentUsers; i++)
		{
			try {
				pluginList.add(new Compare(testSetting, "LoanApplicationLog", "LoanApplication_2_log"));
			}catch(Exception ex)
			{
