...
public class SPFQueueCIPView extends SPFView {
	public SPFQueueCIPView(SPF bpf, String stageName, String subStageName, String viewName, boolean isCategory) {
		...
		try {
			for (int i = 0; i < bpf.getTimeSeries().size(); i++) {
				...
				timeseries.add((RegularTimePeriod) timeP,
						1.0 * stage.getFlowCells().get(i).getCharacteristic(SPF.CHAR_QUEUE_CIP));
			}
