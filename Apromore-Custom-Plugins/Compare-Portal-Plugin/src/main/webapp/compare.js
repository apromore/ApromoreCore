...
ORYX.Editor.prototype.displayMLDifference = function(buttonIndex, type, start, a, b, newTasks, end, start2, end2, greys, annotations) {
    ...
    ORYX.Editor.prototype.repairMLDifference = function() {
        ...
        // Create the "apply" command with undo/redo support, and execute it
        var commandClass = ORYX.Core.Command.extend({
            construct: function(){
                this.oldValue = editorConfig;
                this.newValue = json;
            },
            execute: function(){
                editorConfig = this.newValue;
                json = this.newValue;
                reloadJSON(this.newValue);
                updateRepairs(json);
            },
            rollback: function(){
                editorConfig = this.oldValue;
                json = this.oldValue;
                reloadJSON(this.oldValue);
                updateRepairs(json);
            }
        })
        var command = new commandClass();
        oryxEditor1._getPluginFacade().executeCommands([command]);
    }
}
