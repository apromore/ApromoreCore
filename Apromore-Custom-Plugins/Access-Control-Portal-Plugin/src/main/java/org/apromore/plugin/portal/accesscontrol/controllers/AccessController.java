...
/**
 * Controller for handling share interface Corresponds to components/access/access.zul
 */
...
public class AccessController extends SelectorComposer<Div> {
  ...
  private void destroy() {
    ...
    EventQueues.lookup("accessControl", EventQueues.DESKTOP, true)
        .publish(new Event("onClose", null, null));
  }
  ...
  private void ensureOneOwnerWarning() {
    Messagebox.show(getLabel("ensureOwner_message"), "Apromore", Messagebox.OK, Messagebox.ERROR);
  }
  ...
  private void loadCandidateAssignee() {
    ...
    candidateAssigneeCombobox
        .setModel(ListModels.toListSubModel(candidateAssigneeModel, new Comparator() {
          @Override
  ...
  private void updateArtifacts(String rowGuid) {
    ...
    Set<Usermetadata> userMetadataSet =
        userMetadataService.getUserMetadataByLog(selectedItemId, UserMetadataTypeEnum.FILTER);
    userMetadataSet.addAll(
        userMetadataService.getUserMetadataByLog(selectedItemId, UserMetadataTypeEnum.DASHBOARD));
    for (Usermetadata userMetadata : userMetadataSet) {
      Integer id = userMetadata.getId();
      String name = userMetadata.getName();
      String updatedTime =
          Objects.equals(userMetadata.getUpdatedTime(), null) ? userMetadata.getCreatedTime()
    ...
    artifactListbox.setModel(artifactModel);
    /*
     * // Old method Set<Artifact> selectionSet = new HashSet<Artifact>(); String selectedUserName =
     * securityService.findGroupByRowGuid(rowGuid).getName(); try { Set<Usermetadata>
     * selectedUserMetadataSet = userMetadataService.getUserMetadataByUserAndLog(selectedUserName,
     * selectedItemId, UserMetadataTypeEnum.FILTER);
     * selectedUserMetadataSet.addAll(userMetadataService.getUserMetadataByUserAndLog(
     * selectedUserName, selectedItemId, UserMetadataTypeEnum.DASHBOARD)); for (Usermetadata
     * userMetadata: selectedUserMetadataSet) { Integer id = userMetadata.getId(); String name =
     * userMetadata.getName(); String updatedTime = userMetadata.getUpdatedTime(); UsermetadataType
     * usermetadataType = userMetadata.getUsermetadataType(); Artifact artifact = new Artifact(id,
     * name, updatedTime, usermetadataType); // selectionSet.add(artifact); } } catch (Exception e)
  ...
  public void addCandidateUser(Assignee assignee) {
    ...
    if (selectedItem instanceof UserMetadataSummaryType) {
      showWarning = !userMetadataService
          .canAccessAssociatedLog(((UserMetadataSummaryType) selectedItem).getId(), rowGuid);
    }

    Assignment assignment = new Assignment(assignee.getName(), rowGuid, assignee.getType(),
  ...
  public void onClickCandidateUserAdd() {
    if (Boolean.FALSE.equals(enableUsersList)) {
      ...
      if (assignees != null && assignees.size() == 1 && selectedItem != null
          && selectedItemId != null) {
