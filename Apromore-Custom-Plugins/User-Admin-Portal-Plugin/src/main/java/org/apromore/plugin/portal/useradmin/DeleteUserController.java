...
/**
 * Controller for handling transfer ownership Corresponds to
 * resources/user-admin/transfer-ownership.zul
 */
...
public class DeleteUserController extends SelectorComposer<Window> {
    ...
    public void doAfterCompose(Window win) throws Exception {
        ...
        // If folder solely owned by User-To-Be-Deleted but contains files co-owned, then disable
        // "delete all assets"
        if (!workspaceService.canDeleteOwnerlessFolder(selectedUser)) {
            deleteOptionPurge.setDisabled(true);

            Messagebox.show(
                MessageFormat.format(getLabel("cantDeleteCoOwned_message"), selectedUser.getUsername()),
    ...
    private void loadTransferTo() {
        ...
        // transferToUserMap = users.stream().collect(Collectors.toMap(User::getUsername, user ->
        // user));
