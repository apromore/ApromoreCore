...
public class LogFilterCriterionDurationTotalWaiting extends AbstractLogFilterCriterion {
    ...
    public LogFilterCriterionDurationTotalWaiting(Action action, Containment containment, Level level, String label, String attribute, Set<String> value) {
    ...
    public boolean matchesCriterion(XTrace trace) {
        ...
        long totalWaitingTime = getTotalWaitingTime(trace);
        if(totalWaitingTime == 0) return false;
        else if(totalWaitingTime < greaterThan) return false;
        else if(totalWaitingTime > lesserThan) return false;
        else return true;
    }
    ...
    public String toString() {
        ...
        return super.getAction() + " all traces with a total waiting time between " +
                minString + " to " +
                maxString;
    }
    ...
    private long getTotalWaitingTime(XTrace xTrace) {
