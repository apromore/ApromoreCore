...
//            }
    ...
    public JSONArray generateJSONFromBPMN(VisualizationType type, boolean used_bpmn_size, boolean secondary) throws JSONException {
        ...
        for(BPMNNode node : getNodes(bpmnDiagram)) {
            ...
            if(node instanceof Event) {
                ...
                else if(type == VisualizationType.DURATION) colors = getDurationColor(node, minMaxDur[0], minMaxDur[1], skip);
                else colors = getFrequencyColor(node, minMaxFreq[0], minMaxFreq[1]);
    ...
    private Double[] getMinMaxFrequency(Set<BPMNNode> nodes) {
        double max = 0;
    ...
    private Double[] getMinMaxDuration(Set<BPMNNode> nodes, boolean skip) {
        double max = 0;
        double min = Double.MAX_VALUE;
        for (BPMNNode n : nodes) {
    ...
    private String[] getDurationColor(BPMNNode node, double min, double max, boolean skip) {
        if(max == 0 || max==min) {
            ...
            if(skip && node.getLabel().contains("\\n")) {
                ...
                //node_duration = abs.getLogDFG().getNodeInfoCollector().getNodeDuration(node.getLabel(), params.getPrimaryAggregation());
            	node_duration = abs.getNodePrimaryWeight(node);
            }
