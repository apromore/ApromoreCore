...
/**
 * Despite its name it is testing the conversion of a whole decomposition, so it
 * also tests most of the other YAWL elements.
 * 
 */
public class OryxCompositeTaskHandlerTest extends OryxHandlerTest {
	 ...
	 * @param netId
	 */
	private void doTestCompositeTaskById(String taskId, String netId) {
		...
		YExternalNetElement netElement = context.getNet(parentShape).getNetElement(taskId);

		assertNotNull(netElement);
		assertTrue(netElement instanceof YCompositeTask);

		YCompositeTask task = (YCompositeTask) netElement;
		YCompositeTask originalTask = (YCompositeTask) findNetElementById(taskId, netId);

		if (originalTask.getCancelledBySet().size() > 0) {
			assertEquals("Task ID " + taskId, originalTask.getRemoveSet().size(),
					context.getCancellationSet(context.getNet(parentShape), task).size());
		}
	...
	private void compareNetElements(Map<String, YExternalNetElement> elements1, Map<String, YExternalNetElement> elements2) {

		Comparator<YExternalNetElement> elementComparator = new Comparator<YExternalNetElement>() {

			@Override
			public int compare(YExternalNetElement o1, YExternalNetElement o2) {
		...
		for (YExternalNetElement netElement : sorted1) {
			result1.append(netElement.toXML().replaceAll("<flowsInto>.*?<\\/flowsInto>", ""));
		}

		StringBuilder result2 = new StringBuilder();
		for (YExternalNetElement param : sorted2) {
			result2.append(param.toXML().replaceAll("<flowsInto>.*?<\\/flowsInto>", ""));
		}
	...
	private void compareFlowsInto(List<YExternalNetElement> sorted1, List<YExternalNetElement> sorted2) {
	...
	private void compareParameters(Map<String, YParameter> param1, Map<String, YParameter> param2) {
