 ...
 * 
 * <p>The factory uses the {@link de.hpi.bpel4chor.transformation.factories.Componentizer} to determine patterns
 * in the sequence flow. These patterns are mapped to its BPEL representation
 * and folded to a single task. This task replaces the pattern in the sequence
 * flow and represents the mapped BPEL code.</p>
 * 
 * <p>An instance of this class can only be used for one container 
 * (process, sub-process) in a certain diagram.</p>
 */
public class SequenceFlowFactory {
	...
	/**
	 * Constructor. Initializes the sequence flow factory with the diagram
	 * and the container that contains the sequence flow to be transformed.
	 *  
	 * @param diagram   The diagram the container belongs to.
	 * @param document  The document to create the BPEL elements for.
	 * @param container The container that contains the sequence flow that will 
	 *                  be transformed.
	 * @param output    The {@link de.hpi.bpel4chor.util.Output} to print errors to.
	 */
	...
	/**
	 * <p>Transforms the given start events to intermediate events. The
	 * start events will be removed from the container and the intermediate
	 * events will be added.</p>
	 * 
	 * <p>Start events of trigger type none will not be transformed, because
	 * intermediate events with trigger type none are not allowed. The id,
	 * name and trigger of the start events is assigned to the intermediate
	 * events. The transitions emanating from the start event are changed to 
	 * transitions that emanate from the intermediate events.
	 * If the start event is contained in a process and the createInstance
	 * attribute is true, the createInstance attribute of the 
	 * intermediate event will be set to true, too.</p>
	 * 
	 * @param toTransform    The start events to be transformed to 
	 * 						 intermediate events.
	 * @param createInstance True, if the start events are allowed to 
	 ...
	 * gatway's position in the sequence flow.</p>
	 * 
	 * @param start   The activity to start the with
	 * @param visited The already visited activities.
	 * @param result  This should be an empty list when the method is
	 *                called from the outside. After the call the list will contain
	 *                the found gateways on the paths from start activity to an
	 *                end event.
	 *                
	 * @return True, if an end event was reached or a cycle was detected. 
	 * False, otherwise.
	 */
	 ...
	 * gatway's position in the sequence flow. If a start event does not lead
	 * to an end event an error is added to the output and the result will be 
	 * null.
	 *  
	 * @param startEvents The start events to determine the reachable gateways
	 *                    for.
	 * 
	 ...
	 * gateway type is {@link de.hpi.bpel4chor.model.activities.Gateway#TYPE_OR} an error is added to the output
	 ...
	 * (see {@link #createCombinatingGateway(java.util.List, String)}).</p>
	 * 
	...
	/**
	 * Gets the first gateway that is contained in at least lists 
	 * as possible. This means the first gateway is determined that 
	 * is reachable by the fewest start events. This is a gateway 
	 * that merges the sequence flow starting in these start 
	 * events. All the gateways following this gateway are at least
	 * contained in the lists of these start events, because after this
	 * gateway the lists of the start events are equal.
	 * 
	 * If there is only one key in the map the result is null.
	 *  
	 * @param map The map that maps the start events to the list
	 * 		      of the reachable gateways.
	 * 
	 * @return The first found gateway that is contained in the
	 * fewest lists.
	 */
	private Gateway determineGateway(Map<StartEvent, List<Gateway>> map) {
		...
		for (int i = 2; i <= map.keySet().size(); i++) {
			...
			for (Iterator<List<Gateway>> it = map.values().iterator(); it.hasNext();) {
				List<Gateway> gateways = it.next();
				for (Iterator<Gateway> itGat = gateways.iterator(); itGat.hasNext();) {
	...
	/**
	 * Determines the map value that is the longest list. The map
	 * key of the value must be contained in list of the given keys.
	 *  
	 * @param keys The keys that restrict the set of lists to search for.
	 * @param map  The map to determine the longest list in.
	 * 
	 * @return The determined list.
	 */
	private List<Gateway> getLongestList(List<StartEvent> keys, Map<StartEvent, List<Gateway>> map) {
	 ...
	 * {@link #combineWithGateway(java.util.List, String)}. After that the map is
	 ...
	 * start event trigger and the container it is contained in:
	 * <ul>
	 ...
	 * </ul>
	 * 
	 * @param start The start event to be checked.
	 * 
	...
	/**
	 * Checks if the given start events are valid. The validity depends on 
	 * the number of start events, on the start event trigger and on the 
	 * container it is contained in:
	 * <ul>
	 * 	<li> Each process or sub-process must contain a least one start event
	 *  <li> Message and Timer handlers are not allowed to contain multiple 
	 *       start events
	 *  <li> If there are multiple start events they are not allowed to be
	 *  	 non-triggered (timer triggers are only allowed in timer handlers,
	 *       that do not allow multiple start events)
	 *  <li> see {@link #isValidStartEvent(de.hpi.bpel4chor.model.activities.StartEvent)}
	 * </ul>
	 * 
	 * @param startEvents The start events to be checked
	 * 
	 ...
	 * 
	 * <p>The existing end events are removed from the container. The incoming
	...
	/**
	 * <p>Checks if the sequence flow is trivial. A trivial sequence flow
	 * has one start and one end event. Between these events there is 
	 * at most one activity (no gateway). If there is no activity in between
	 * the start event has to be a message triggered start event.
	 * 
	 * @param start The start event of the sequence flow.
	 * @param end The end event of the sequence flow.
	 * 
	 * @return True if the sequence flow of the container is trivial, 
	 * false otherwise.
	 */
	...
	/**
	 * Maps a task to its BPEL4Chor representation. If the task
	 * represents a looping activity a loop element is created 
	 * containing the mapped task. The mapping of a folded task
	 * is just the BPEL element it represents.
	 * 
	 * @param task The task to be mapped.
	 * 
	 * @return The BPEL4Chor element the task represents or null if
	 ...
	 * activity is a looping activity a looping element will be created 
	 * (see {@link StructuredElementsFactory#createLoopElement(de.hpi.bpel4chor.model.activities.Activity, org.w3c.dom.Element)}),
	 * otherwise a scope element will be created (see 
	 * {@link StructuredElementsFactory#createScopeElement(de.hpi.bpel4chor.model.activities.BlockActivity)}).
	 ...
	 * 
	 * @param blockActivity The block activity to be mapped.
	 * 
	...
	private Element mapBlockActivity(BlockActivity blockActivity) {
		if (blockActivity.getLoop() != null) {
			return this.structuredFactory.createLoopElement(blockActivity, null);
		}
	...
	private boolean isValidLocation(IntermediateEvent event) {
		if (!event.getTriggerType().equals(IntermediateEvent.TRIGGER_COMPENSATION)) {
			return true;
		}
		// compensate event can only located in fault, compensation or termination handlers
		if (event.getParentContainer() instanceof SubProcess) {
			BlockActivity parent = 
				((SubProcess)event.getParentContainer()).getBlockActivity();
	 ...
	 * located in a valid container. If the container is not valid and erros 
	 * is added to the output and the result will be null. A rethrow element
	 * can only be generated if the error event is located in a fault handler.
	 * 
	 * @param event The intermediate event to be mapped.
	 * 
	 * @return The BPEL4Chor element the event represents or null if
	 * the event could not be mapped.
	 */
	 ...
	 * {@link #isTrivial(de.hpi.bpel4chor.model.activities.StartEvent, de.hpi.bpel4chor.model.activities.EndEvent)}) to its BPEL4Chor elements it
	 ...
	 * a target element will be created in the targets element. 
	 * The created sources and targets will be added to the given element.
	 * 
	 * @param act      The activity to create the so.urces and targets for.
	 * @param element  The element to add the created sources and targets to
	 * @param links    The links the activity can be source or target of.
	 * @param joinCond The join condition for the targets element.
	 */
	private void createSourcesAndTargets(Activity act, Element element, List<Link> links, Expression joinCond) {
		if (links == null) {
			return;
		}
		...
		Element joinCondElement = this.supportingFactory.createExpressionElement(
	...
	/**
	 * Maps an activity to its BPEL4Chor representation. The mapped BPEL4Chor
	 * element will not define a join condition. If the activity is not a block
	 * activity, task or intermediate event, an error is added to the output
	 ...
	 * 
	 * @return The created BPEL4Chor element the activity represents or null
	 * if the activity could not be mapped.
	 */
	public Element mapActivity(Activity act, List<Link> links) {
		return mapActivity(act, links, null);
	}
	
	/**
	 * Maps an activity to its BPEL4Chor representation. If the activity
	 * is not a block activity, task or intermediate event, an error is added
	 * to the output and the result will be null.
	 * 
	 * @param act      The activity to be mapped.
	 * @param links    Links that are defined in the context of the activity.
	 ...
	 * purpose the activities of the component are mapped to BPEL4Chor. 
	 * After that a "sequence" element will be created that contains these
	 * mapped BPEL4Chor elements.
	 *  
	 * @param comp  The sequence component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	...
	/**
	 * Maps an attached events component. Since the mapping of 
	 * attached events is already included in the mapping of the
	 * activity the events are attached to just the activity
	 * needs to be mapped.
	 *  
	 * @param comp  The attached events component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	 * @return The created activity element or null if the activity could
	 * not be mapped.
	 */
	...
	/**
	 * Maps a flow component to its BPEL4Chor representation. For this
	 * purpose the activities of the component are mapped to BPEL4Chor. 
	 * After that an "flow" element will be created that contains these
	 * mapped BPEL4Chor elements.
	 * 
	 * Directed sequence flows from the source gateway to the sink 
	 * gateway are mapped to an empty activity.
	 *  
	 * @param comp  The if component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	...
	/**
	 * Maps an if component to its BPEL4Chor representation. For this
	 * purpose the activities of the component are mapped to BPEL4Chor. 
	 * After that an "if" element will be created that contains these
	 * mapped BPEL4Chor elements.
	 * 
	 * <p>Directed sequence flows from the source gateway to the sink 
	 * gateway are mapped to an empty activity.</p>
	 * 
	 * <p>If the source gateway has an outgoing sequence flow
	 * that is not conditional or a default flow, an error will be
	 * added to the output and the result will be null. This will also be done if 
	 * there are multiple default flows attached to the source gateway.</p>
	 *  
	 * @param comp  The if component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	...
	/**
	 * Maps a pick component to its BPEL4Chor representation. For this
	 * purpose the activities of the component are mapped to BPEL4Chor. 
	 * After that a "pick" element will be created that contains these
	 * mapped BPEL4Chor elements.
	 * 
	 * <p>Directed sequence flows from a branch activity to the sink 
	 * gateway are mapped to an empty activity.</p>
	 *  
	 * @param comp  The pick component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	 ...
	 * 
	 * @param source     The activity whose successors will be checked.
	 * @param successors A list with the successors to be checked.
	 * 
	 ...
	 * 
	 * @param target       The activity whose predecessors will be checked.
	 * @param predecessors A list with the predecessors to be checked.
	 * 
	 * @return True, if all predecessors of the activity are contained in the list,
	 * false otherwise
	 */
	...
	private Expression getRefinedInclCond() {
		...
		result.setExpressionLanguage("urn:oasis:names:tc:wsbpel:2.0:sublang:xpath1.0");
	 ...
	 * purpose the activities of the component are mapped to BPEL4Chor. 
	 * After that a "repeatUntil" element will be created that contains these
	 * mapped BPEL4Chor elements.
	 * 
	 * <p>If the sink object does not have conditional outgoing sequence flows
	 * an error is added to the output and the result will be null. This
	 * will also be done if there is not exaclty one activity (except source
	 * and sink object) in the component.</p>
	 *  
	 * @param comp  The repeat component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	 ...
	 * purpose the activities of the component are mapped to BPEL4Chor. 
	 * After that a "while" element will be created that contains these
	 * mapped BPEL4Chor elements. The "while" element will be contained
	 * in a "sequence" element.
	 * 
	 * <p>If the sink object does not have conditional outgoing sequence flows
	 * an error is added to the output and the result will be null. This
	 * will also be done if there are not exactly two activities (except source
	 * and sink object) in the component.</p>
	 *  
	 * @param comp  The repeat-while component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	 * @return The created "sequence" element.
	 */
	...
	/**
	 * Maps a while component to its BPEL4Chor representation. For this
	 * purpose the activity of the component is mapped to BPEL4Chor. 
	 * After that a "while" element will be created that contains this
	 * mapped BPEL4Chor element.
	 * 
	 * <p>If the sink object does not have conditional outgoing sequence flows
	 * an error is added to the output and the result will be null. This
	 * will also be done if there is not exaclty one activity (except source
	 * and sink object) in the component.</p>
	 *  
	 * @param comp  The while component to be mapped.
	 * @param links Links that are defined in the context of the component.
	 * 
	 ...
	 * purpose the activities of the component are mapped to BPEL4Chor. 
	 * After that a "flow" element will be created that contains this
	 * mapped BPEL4Chor elements. The flow starts with an empty activity.
	 * This activity is the source of the links that lead to the mapped
	 * BPEL4Chor elements.
	 *  
	 * @param comp  The special-flow component to be mapped.
	 * 
	 * @return The created "flow" element.
	 */
	...
	/**
	 * Folds a component to a single folded task. 
	 * There is no need to define links in the mapped elements 
	 * of the component (see {@link #foldComponent(de.hpi.bpel4chor.transformation.factories.Component, java.util.List)}).
	 * 
	 * @param comp The component to be folded
	 * 
	...
	/**
	 * Creates and inserts a folded task into the sequnce flow of 
	 * the container replacing the given component. The folded
	 * task holds the given element. The activities and transitions
	 * of the component will be removed from the container.
	 * 
	 * @param comp    The component to be folded.
	 * @param element The BPEL4Chor element represented by the folded task.
	 * 
	...
	/**
	 * Folds a component to a single folded task. For this purpose the
	 * component is mapped to its BPEL4Chor representation. After that the
	 * activities and transitions of the component will be removed from the
	 * container. A folded task will be created that holds the mapped BPEL4Chor
	 * element and is inserted at the location of the component. 
	 * 
	 * <p>If the component is a quasi-component it will only be refined.
	 * There will not be created a folded task.</p>
	 * 
	 * @param comp  The component to be folded
	 * @param links Links that are defined in the context of the component.
	 * 
	 * @return The folded task that holds a mapped BPEL4Chor element or null,
	 ...
	 * representation. 
	 * 
	 * <p>First multiple start and end events will be combined. After that 
	 * components will be identified and folded (see 
	 * {@link de.hpi.bpel4chor.transformation.factories.Componentizer#getNextComponent()}).
	 * If there are still activities left but no component can be found and
	 * the sequence flow is not trivial, an error will be added to the output
	 * and the result will be null.</p>
	 * 
