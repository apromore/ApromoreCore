...
public class ReachabilityGraph<Diagram, FlowObject, Marking> extends
    ...
    public ReachabilityTransition<FlowObject, Marking> addEdge(
        ...
        return new ReachabilityTransition<>(this, s, t);
    }
    ...
    public void addMarking(Marking m) {
        ...
        this.addVertex(new ReachabilityNode<>(m));
    }
    ...
    public void add(Marking fromMarking, Marking toMarking, FlowObject node) {
        ...
        if (this.getEdgesWithSourceAndTarget(fromMarkingNode, toMarkingNode).size() == 0) {
            ReachabilityTransition<FlowObject, Marking> transition = this.addEdge(fromMarkingNode, toMarkingNode);
    ...
    public List<Marking> getMarkings() {
        List<Marking> markings = new LinkedList<>();
    ...
    public List<FlowObject> getFlowObjects() {
        List<FlowObject> flowObjects = new LinkedList<>();
    ...
    public List<Marking> getPredecessors(Marking m) {
        List<Marking> list = new LinkedList<>();
    ...
    public List<ReachabilityPath<FlowObject, Marking>> getPathsFromRoot(Marking toMarking) {
        List<ReachabilityPath<FlowObject, Marking>> paths = new LinkedList<>();
        for (Marking marking : this.getRoots()) {
    ...
    public Marking getSuccessor(Marking m, FlowObject fo) {
        for (ReachabilityTransition<FlowObject, Marking> trans : this.getOutgoingEdges(this.findByMarking(m))) {
            if (trans.getFlowObject() == fo) {
                return (trans.getTarget()).getMarking();
            }
        }
        return null;
    }
    ...
    public List<Marking> getRoots() {
        List<Marking> list = new LinkedList<>();
        for (ReachabilityNode<Marking> node : directedGraphAlgorithms.getInputVertices(this)) {
    ...
    public List<Marking> getLeaves() {
        List<Marking> list = new LinkedList<>();
        for (ReachabilityNode<Marking> node : directedGraphAlgorithms.getOutputVertices(this)) {
            list.add(node.getMarking());
        }
        return list;
    }
