...
/**
 * Stencilset compressor.
 *
 * This was originally a command-line build utility, part of the Signavio process model editor.
 * It has been modified to function as a Maven plugin within the Apromore build process.
 */
@Mojo(name="compress")
public class SSCompressor extends AbstractMojo {

	@Parameter(property="compress.ssDirString", defaultValue="${project.basedir}")
	private String ssDirString;

	@Parameter(property="compress.outDirString", defaultValue="${project.basedir}/target/sscompressor")
        private String outDirString;

	public void execute() throws MojoExecutionException, MojoFailureException {
	    try{
		getLog().info("Compressing stencilsets from " + ssDirString);
		getLog().info("Compressing stencilsets into " + outDirString);
                
		//get stencil set configuration file
		File ssConf = new File(ssDirString + "/stencilsets.json"); 
		
		if(!ssConf.exists())
			throw new MojoFailureException("File " + ssDirString + "/stencilsets.json does not exist.");
		...
		for(int i = 0; i < jsonObj.length(); i++) {
			...
			if (ssObj.has("basestencilset") && ssObj.has("extensionstointegrate")){
				...
				if (ssObj.has("uri")) {
					...
					File ssFile = new File(outDirString + ssUri);
					
					if (ssFile.exists()){
						throw new MojoFailureException("The stencil set (" + ssDirString + ssUri + ") that should be created does already exist.");
					}
					
					ssFile  = createFile(outDirString, ssUri);
					
					File baseSsFile = getStencilSet(ssObj.getString("basestencilset"), ssConf, ssDirString);
					getLog().info("Base stencil set: " + baseSsFile);
					String baseSsString = readFile(baseSsFile).toString();
					getLog().info("Pop!");
					JSONObject BaseSsJson = new JSONObject( baseSsString );
					getLog().info("Bop!");
					...
					for (int j = 0; j < extensions.length(); j++) {
						...
						getLog().info("Reading extension file " + extensionFile);
				...
				if(!ssFile.exists())
					throw new MojoFailureException("Stencil set " + ssDirString + ssUri + " that is referenced in stencil set configuration file does not exist.");
				...
				for (int j = 0; j < extensions.length(); j++) {
					...
					if (ssJson.getString("namespace").equals(extension.getString("extends"))) {
						...
						getLog().warn("Could not merge "  + ssJson.getString("namespace") + " with extension " + extension.getString("namespace"));
						
					}
					
				}
				ssFile = new File(outDirString + ssUri);
			...
			if(ssObj.has("uri")) {
				...
				File ssNoSvgFile = new File(outDirString + ssUri.substring(0, pIdx) + "-nosvg" + ssUri.substring(pIdx));
				...
				while(matcher.find()) {
					...
					if(!svgFile.exists())
						throw new MojoFailureException("SVG File " + svgFile.getPath() + " does not exists!. Compressing stencil sets aborted!");
				...
				*/
				...
				getLog().info("Compressed stencil set file " + ssFile.getCanonicalPath());
			}
		}
            } catch (Exception e) {
		e.printStackTrace();
		throw new MojoExecutionException("Unable to compress stencil sets: " + e.getMessage(), e);
	    }
	...
	private /*static*/ JSONObject createJsonFor(JSONObject newStencilSetData, JSONObject baseStencilSetData, List<JSONObject> extensions) throws JSONException {
		...
		for (JSONObject extension : extensions) {
			if (baseStencilSetData.getString("namespace").equals(extension.getString("extends"))) {
				...
				getLog().warn("[Warning] could not merge "  + baseStencilSetData.getString("namespace") + " with extension " + extension.getString("namespace"));
			}
		}
