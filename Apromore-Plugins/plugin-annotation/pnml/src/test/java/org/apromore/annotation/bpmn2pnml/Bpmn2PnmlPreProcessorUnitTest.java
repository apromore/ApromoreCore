package org.apromore.annotation.bpmn2pnml;
 ...
 */
public class Bpmn2PnmlPreProcessorUnitTest {

    Bpmn2PnmlPreProcessor bpmn2PnmlPreProcessor;

    @Test
    public void testProcessAnnotation() throws Exception {
        bpmn2PnmlPreProcessor = new Bpmn2PnmlPreProcessor();

        CanonicalProcessType cpf = buildCPF();
        AnnotationsType anf = buildANF();

        bpmn2PnmlPreProcessor.processAnnotation(cpf, anf);

        assertThat(anf.getAnnotation().size(), equalTo(3));

        assertThat(((GraphicsType) anf.getAnnotation().get(0)).getPosition().size(),  equalTo(1));
        assertThat(((GraphicsType) anf.getAnnotation().get(0)).getPosition().get(0).getX().doubleValue(),  equalTo(80.5));
        assertThat(((GraphicsType) anf.getAnnotation().get(0)).getPosition().get(0).getY().doubleValue(),  equalTo(80.5));

        assertThat(((GraphicsType) anf.getAnnotation().get(1)).getPosition().size(),  equalTo(1));
        assertThat(((GraphicsType) anf.getAnnotation().get(1)).getPosition().get(0).getX().doubleValue(),  equalTo(180.5));
        assertThat(((GraphicsType) anf.getAnnotation().get(1)).getPosition().get(0).getY().doubleValue(),  equalTo(180.5));
