...
public abstract class BaseVarMappingHandler extends YAWLConversionHandler<VarMappingFactsType, TaskType> {
    ...
    protected List<ObjectType> calculateUsedTaskVariables(final String xQuery, final TaskType task) {
        final ArrayList<ObjectType> usedVariables = new ArrayList<ObjectType>();
        // This will capture most of the variables that are used in a YAWL output mapping!
        final Pattern p = Pattern.compile("/" + task.getOriginalID() + "/([^/]*)/text()");
        final Matcher m = p.matcher(xQuery);
        while (m.find()) {
            final String varName = m.group(1);
            final ObjectType object = getContext().getObjectByName(varName, task);
            if (object != null) {
                usedVariables.add(object);
            }
        }
        if (usedVariables.isEmpty()) {
            // Try whole XML element without 'text()'
            final Pattern p2 = Pattern.compile("/" + task.getOriginalID() + "/([^/]*)");
            final Matcher m2 = p2.matcher(xQuery);
            while (m2.find()) {
                final String varName = m2.group(1);
                final ObjectType object = getContext().getObjectByName(varName, task);
