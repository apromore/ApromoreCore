...
public class ProcessListboxController extends BaseListboxController {
	...
	public ProcessListboxController(MainController mainController) {
		super(mainController, "macros/listbox/processSummaryListbox.zul",
				new ProcessSummaryItemRenderer());

		this.columnScore = (Listheader) this.getListBox().getFellow(
				"columnScore");
		...
		getListBox().addEventListener("onSelect", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				if (getListBox().getSelectedItems().size() == 1) {
					getMainController().displayProcessVersions(
							(ProcessSummaryType) getListModel().getSelection()
									.iterator().next());
				} else {
	...
	public void displayProcessSummaries(ProcessSummariesType processSummaries,
			Boolean isQueryResult, ProcessSummaryType processQ,
			VersionSummaryType versionQ) {
