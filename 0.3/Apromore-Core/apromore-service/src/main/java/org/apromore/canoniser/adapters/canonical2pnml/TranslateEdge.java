...
public class TranslateEdge {
	...
	public void translateArc(EdgeType edge) {
		...
		data.put_id_map(edge.getId(), BigInteger.valueOf(ids));
		...
		data.put_pnmlRefMap(BigInteger.valueOf(Long.valueOf(arc.getId())), arc);
		data.put_originalid_map(BigInteger.valueOf(Long.valueOf(arc.getId())),
				edge.getOriginalID());
