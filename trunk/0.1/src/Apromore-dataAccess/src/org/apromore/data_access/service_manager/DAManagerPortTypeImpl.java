...
import org.apromore.data_access.model_manager.UpdateProcessSummariesInputMsgType;
import org.apromore.data_access.model_manager.UpdateProcessSummariesOutputMsgType;
import org.apromore.data_access.model_manager.UpdateProcessSummariesType;
...
@javax.jws.WebService(
	...
	public UpdateProcessSummariesOutputMsgType updateProcessSummaries(UpdateProcessSummariesInputMsgType payload) { 
        LOG.info("Executing operation updateProcessSummaries");
        System.out.println(payload);
        UpdateProcessSummariesOutputMsgType res = new UpdateProcessSummariesOutputMsgType ();
		ResultType result = new ResultType();
		res.setResult(result);
        try {
        	List<UpdateProcessSummaryType> processVersions = payload.getUpdateProcessSummaries().getUpdateProcessSummary();
        	for (int i=0; i<processVersions.size(); i++){
        		ProcessDao.getInstance().updateProcess (processVersions.get(i));
        	}
			result.setCode(0);
			result.setMessage("");
        } catch (Exception ex) {
	...
	public ReadCanonicalAnfOutputMsgType readCanonicalAnf(ReadCanonicalAnfInputMsgType payload) { 
		...
		try {
			...
			String anf = ProcessDao.getInstance().getAnnotation(payload.getProcessId(), 
					payload.getVersion());
