...
public class ProcessDetailsController extends BaseController {
	...
	public ProcessDetailsController(MainController mainController,
			BaseListboxController processtable) throws JAXBException,
			ExceptionDao {
		this.mainC = mainController;
		this.processTableC = processtable;
		this.processDetailsW = (Window) mainC.getFellow("processdetailscomp")
				.getFellow("processdetails");

		/**
		 * get the components
		 */
		this.nameT = (Textbox) processDetailsW.getFellow("nameT");
		this.originalLanguageT = (Textbox) processDetailsW
				.getFellow("originalLanguageT");
		...
		this.creationDateD = (Datebox) processDetailsW
				.getFellow("creationDateD");
	...
	public void displayProcessVersionDetails(ProcessSummaryType process,
			VersionSummaryType version, Boolean displayed)
		...
		if (this.originalLanguageH.size() == 1) {
			this.originalLanguageT.setValue((String) this.originalLanguageH
					.get(0).getValue());
		} else {
		...
		if (this.lastUpdateH.size() == 1) {
			this.lastUpdateD
					.setValue((Date) this.lastUpdateH.get(0).getValue());
		} else {
		...
		if (this.creationDateH.size() == 1) {
			...
			this.creationDateD.setValue((Date) this.creationDateH.get(0)
					.getValue());
		} else {
		...
		if (this.versionNameH.size() == 1) {
			this.versionT
					.setValue((String) this.versionNameH.get(0).getValue());
		} else {
