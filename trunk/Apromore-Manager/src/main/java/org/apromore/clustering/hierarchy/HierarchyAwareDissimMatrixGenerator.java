...
public class HierarchyAwareDissimMatrixGenerator implements DissimilarityMatrix {
        ...
        for (int p = 0; p < roots.size(); p++) {
            ...
            if (p < roots.size() - 1) {
                for (int q = p + 1; q < roots.size(); q++) {
                    List<Integer> h2 = containmentRelation.getHierarchy(roots.get(q));
    ...
    private void computeDissim(Integer fid1, Integer fid2) {
        ...
        int fid2Index = containmentRelation.getFragmentIndex(fid2);

        if (!containmentRelation.areInContainmentRelation(fid1Index, fid2Index)) {
    ...
    private SimpleGraph getSimpleGraph(Integer fragId) {
        ...
        if (graph == null) {
            try {
                Canonical cpfGraph = composerService.compose(fragmentVersionRepository.findOne(fragId));
