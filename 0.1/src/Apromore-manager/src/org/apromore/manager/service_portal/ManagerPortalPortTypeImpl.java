...
import org.apromore.manager.model_toolbox.CanonicalsType;
...
@javax.jws.WebService(
	...
	searchForSimilarProcesses(org.apromore.manager.model_portal.SearchForSimilarProcessesInputMsgType payload) { 
		...
		try {
			...
			skipeweight = null;      	
			for (ParameterType p : payload.getParameters().getParameter()) {
				if ("modelthreshold".compareTo(p.getName())==0) {
					modelthreshold = p.getValue();
				}
				if ("labelthreshold".compareTo(p.getName())==0) {
					labelthreshold = p.getValue();
				}
				if ("contextthreshold".compareTo(p.getName())==0) {
					contextthreshold = p.getValue();
				}
				if ("skipnweight".compareTo(p.getName())==0) {
					skipnweight = p.getValue();
				}
				if ("subnweight".compareTo(p.getName())==0) {
					subnweight = p.getValue();
				}
				if ("skipeweight".compareTo(p.getName())==0) {
					skipeweight = p.getValue();
				}
			}
			RequestToToolbox req = new RequestToToolbox();
			org.apromore.manager.model_portal.CanonicalsType canonicals = req.SearchForSimilarProcesses(
					processId, algo, modelthreshold, labelthreshold, contextthreshold, skipnweight, subnweight, skipeweight);
