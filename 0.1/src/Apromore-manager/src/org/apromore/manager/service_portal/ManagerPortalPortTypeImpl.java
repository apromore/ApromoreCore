...
import org.apromore.manager.model_portal.CpfIdType;
...
@javax.jws.WebService(
	...
	public org.apromore.manager.model_portal.MergeProcessesOutputMsgType mergeProcesses(org.apromore.manager.model_portal.MergeProcessesInputMsgType payload) { 
		...
		System.out.println(payload);
		try {
			ToolboxManagerService toolboxService = new ToolboxManagerService();
			ToolboxManagerPortType toolbox = toolboxService.getToolboxManager();

			org.apromore.manager.model_toolbox.MergeProcessesInputMsgType reqToToolbox = new org.apromore.manager.model_toolbox.MergeProcessesInputMsgType();
			reqToToolbox.setProcessName(payload.getProcessName());
			reqToToolbox.setUsername(payload.getUsername());

			// MCF reqToToolbox.setAlgorithm(payload.getAlgorithm());
			org.apromore.manager.model_toolbox.ParametersType parametersToolbox = new org.apromore.manager.model_toolbox.ParametersType();

			for (ParameterType p : payload.getParameters().getParameter()) {
				org.apromore.manager.model_toolbox.ParameterType param = new org.apromore.manager.model_toolbox.ParameterType();
				param.setName(p.getName());
				param.setValue(p.getValue());
				parametersToolbox.getParameter().add(param);
			}

			// MCF reqToToolbox.setParameters(parametersToolbox);

			// processes
			org.apromore.manager.model_toolbox.CpfIdsType ids = new org.apromore.manager.model_toolbox.CpfIdsType();
			for (CpfIdType t : payload.getCpfIds().getCpfId()) {
				org.apromore.manager.model_toolbox.CpfIdType id = new org.apromore.manager.model_toolbox.CpfIdType();
				// TODO should set the version also
				id.setProcessId(t.getProcessId());
				ids.getCpfId().add(id);
			}
			reqToToolbox.setCpfIds(ids);
			reqToToolbox.setVersionName(payload.getVersionName());

			org.apromore.manager.model_toolbox.MergeProcessesOutputMsgType respFromToolbox = toolbox
			.mergeProcesses(reqToToolbox);
			...
			_returnResult.setMessage("Message-1601341583");
			_returnResult.setCode(Integer.valueOf(-308276038));
			...
			_returnProcessSummary.setDomain(respFromToolbox.getProcessSummary().getDomain());
			_returnProcessSummary.setId(respFromToolbox.getProcessSummary().getId());
			_returnProcessSummary.setLastVersion(respFromToolbox.getProcessSummary().getLastVersion());
			_returnProcessSummary.setName(respFromToolbox.getProcessSummary().getName());
			_returnProcessSummary.setOwner(respFromToolbox.getProcessSummary().getOwner());
			_returnProcessSummary.setRanking(respFromToolbox.getProcessSummary().getRanking());
	...
	searchForSimilarProcesses(org.apromore.manager.model_portal.SearchForSimilarProcessesInputMsgType payload) { 
		...
		org.apromore.manager.model_portal.ResultType result = 
			new org.apromore.manager.model_portal.ResultType();
		res.setResult(result);
		org.apromore.manager.model_portal.SearchForSimilarProcessesOutputMsgType _return = 
			new org.apromore.manager.model_portal.SearchForSimilarProcessesOutputMsgType();
		try {
/*
			ToolboxManagerService toolboxService = new ToolboxManagerService();
			ToolboxManagerPortType toolbox = toolboxService.getToolboxManager();

			org.apromore.manager.model_toolbox.SearchForSimilarProcessesInputMsgType reqToToolbox = 
				new org.apromore.manager.model_toolbox.SearchForSimilarProcessesInputMsgType();
			reqToToolbox.setAlgorithm(payload.getAlgorithm());

			String algoToolbox = payload.getAlgorithm();
			org.apromore.manager.model_toolbox.ParametersType parametersToolbox = 
			...
			reqToToolbox.setParameters(parametersToolbox);
			reqToToolbox.setProcessId(payload.getProcessId());
			reqToToolbox.setVersionName(payload.getVersionName());
			List<org.apromore.manager.model_portal.CanonicalsType>
			org.apromore.manager.model_toolbox.SearchForSimilarProcessesOutputMsgType respFromToolbox = 
				toolbox.searchForSimilarProcesses(reqToToolbox);


			org.apromore.manager.model_portal.ResultType _returnResult = 
				new org.apromore.manager.model_portal.ResultType();
			_returnResult.setMessage("Message-1601341583");
			_returnResult.setCode(Integer.valueOf(-308276038));
			_return.setResult(_returnResult);
			java.util.List<org.apromore.manager.model_portal.CanonicalsType> _returnCanonicals = 
				new java.util.ArrayList<org.apromore.manager.model_portal.CanonicalsType>();
			for (org.apromore.manager.model_toolbox.CanonicalsType c : respFromToolbox.getCanonicals()) {

				org.apromore.manager.model_portal.CanonicalsType _returnCanonicalsVal1 = 
					new org.apromore.manager.model_portal.CanonicalsType();
				_returnCanonicalsVal1.setProcessId(c.getProcessId());
				_returnCanonicalsVal1.setVersionName(c.getVersionName());
				_returnCanonicalsVal1.getCpf().addAll(c.getCpf());
