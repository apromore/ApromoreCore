...
public class CaseLengthFilter {
    ...
    private static boolean conformRule(PTrace trace, LogFilterRule logFilterRule) {
        long traceLength = trace.getActivityInstances().size();

        long utilRangeFrom = 0, utilRangeTo = 0;
        for (RuleValue ruleValue : logFilterRule.getPrimaryValues()) {
            OperationType operationType = ruleValue.getOperationType();
            if (operationType == OperationType.GREATER_EQUAL) utilRangeFrom = ruleValue.getLongValue();
            if (operationType == OperationType.LESS_EQUAL) utilRangeTo = ruleValue.getLongValue();
        }

        return traceLength >= utilRangeFrom && traceLength <= utilRangeTo;
    }
}
