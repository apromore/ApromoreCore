...
public abstract class AbstractLogFilterRuleValidator {
    ...
    protected static LogFilterRule replaceLongValues(LogFilterRule logFilterRule, LongArrayList lal) {
        ...
        if (validFrom == lal.min() && validTo == lal.max()) return null;

        return replaceLongValues(logFilterRule, validFrom, validTo);
    }

    protected static LogFilterRule replaceLongValues(LogFilterRule logFilterRule, long from, long to) {
        for (RuleValue ruleValue : logFilterRule.getPrimaryValues()) {
            OperationType operationType = ruleValue.getOperationType();
