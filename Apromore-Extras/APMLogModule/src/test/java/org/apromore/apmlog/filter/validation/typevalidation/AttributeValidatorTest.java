...
public class AttributeValidatorTest extends FilterRuleValidatorTest {
    ...
    private LogFilterRule getMultiValueRule(FilterType filterType, String key, Set<String> values, String section) {
        RuleValue r1 = new RuleValue(filterType, OperationType.EQUAL, key, values);
        return getRuleImpl(filterType, key, r1, section);
    }
    ...
    public void testMultiValueEventAttrValidator() throws Exception {
        LogFilterRule rule = getMultiValueRule(FilterType.CASE_EVENT_ATTRIBUTE, XESAttributeCodes.CONCEPT_NAME,
                Set.of("Proceed order", "Prepare package",
                        "Tranfer items from warehouse 2 one 'two three four five' and six"), "event");
        List<LogFilterRule> criteria = List.of(rule);
