...
public class SecuritySsoHelper {
  ...
  public static String getEnvEncKey() {
    String encPassphStr = ApromoreEnvUtils.getEnvPropValue(SYMMETRIC_KEY_SECRET_ENV_KEY,
        "encryption passphrase could *not* be attained from environment");
  ...
  public static String getAppAuthHeader() {
    ...
    final String appAuthHeader = securityContextHolderAwareRequestWrapper.getHeader("App_Auth");
  ...
  public static String getSsoUsername(final String urlDecoded, String encKey) throws Exception {
    ...
    final String usernameParsed =
        usernameKeyValuePair.substring(usernameKeyValuePair.indexOf("=") + 1);
    LOGGER.info("\n\n>>>>> >>>>> >>>>> usernameParsed: {}", usernameParsed);

    return usernameParsed;
  }

  public static PortalSessionQePair initialiseKeycloakUser(final String usernameParsed,
      final UserService userService, final SecurityService securityService,
      final ManagerService managerService, final ConfigBean config,
    ...
    try {
      ...
      portalSessionQePair = new PortalSessionQePair(new PortalSession(mainController),
          EventQueues.lookup(Constants.EVENT_QUEUE_REFRESH_SCREEN, EventQueues.SESSION, false));
  ...
  public static boolean logoutCurrentUser(final UserType currentUser, final Event event,
      final ConfigBean config, final ManagerService managerService) {
    ...
    try {
      ...
      // Regardless of whether the server is configured to use Keycloak or not, or whether for some
      // exceptional
      // case Keycloak server session logout fails - the user should be logged out of
      // ZK/servlet/Spring security.
      // Even, in the exceptional case (i.e. the Keycloak logout fails), the Jwt is timed to maximum
      // period -
