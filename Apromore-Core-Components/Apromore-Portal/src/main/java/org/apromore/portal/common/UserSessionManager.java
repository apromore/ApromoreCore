...
public abstract class UserSessionManager {
  ...
  public static void setCurrentUser(UserType user) {
    ...
    LOGGER.debug("Set user session current user attribute to {}",
  ...
  public static void initializeUser(ManagerService manager, ConfigBean config, UserType userType,
      User userAcct) {
  ...
  private static UserType constructUserType(String username, ConfigBean config)
    ...
    String[] attributes = {config.getLdapEmailAttribute(), config.getLdapFirstNameAttribute(),
        config.getLdapLastNameAttribute()};
    constraints.setReturningAttributes(attributes);
    NamingEnumeration results = context.search(config.getLdapUserContext(),
        String.format("%s=%s", config.getLdapUsernameAttribute(), username), constraints);
    ...
    // membership.setPassword("password"); // Beware that if you specify a value here, it (in
    // addition to the LDAP password) can be used to authenticate
  ...
  private static String findAttributeFieldByName(Attributes attributes, String fieldName)
      throws NamingException {
  ...
  /*
   * public static void setCurrentFolder(FolderType folder) { setAttribute(CURRENT_FOLDER, folder);
   * if (folder != null && getMainController() != null) {
   * getMainController().setBreadcrumbs(folder.getId()); } }
   * 
   * public static FolderType getCurrentFolder() { FolderType folder = (FolderType)
   * getAttribute(CURRENT_FOLDER); if (folder == null) { folder = new FolderType(); folder.setId(0);
   * folder.setFolderName("Home"); setCurrentFolder(folder); }
   * 
   * return folder; }
   * 
   ...
   * 
   * @SuppressWarnings("unchecked") public static List<Integer> getSelectedFolderIds() { if
   * (getAttribute(SELECTED_FOLDER_IDS) != null) { return (List<Integer>)
   * getAttribute(SELECTED_FOLDER_IDS); }
   * 
   ...
   * 
   * @SuppressWarnings("unchecked") public static List<Integer> getSelectedProcessIds() { if
   * (getAttribute(SELECTED_PROCESS_IDS) != null) { return (List<Integer>)
   * getAttribute(SELECTED_PROCESS_IDS); }
   * 
   * return new ArrayList<>(); }
   * 
   * public static void setPreviousFolder(FolderType folder) { setAttribute(PREVIOUS_FOLDER,
   * folder); }
   * 
   * public static FolderType getPreviousFolder() { if (getAttribute(PREVIOUS_FOLDER) != null) {
   * return (FolderType) getAttribute(PREVIOUS_FOLDER); }
   * 
   * return null; }
   * 
   * public static void setTree(List<FolderType> folders) { setAttribute(TREE, folders); }
   * 
