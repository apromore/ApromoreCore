...
public class ControlFlowDriftDetector_EventStream implements ControlFlowDriftDetector{
	...
	private InductiveMinerConfig IMConfig = InductiveMinerConfig.IM;
	private FeatureExtractionConfig FEConfig = FeatureExtractionConfig.WB;
	private NoiseFilterConfig noiseFilterConfig = NoiseFilterConfig.Removal;
	
//	MiningParameters miningParameters = new MiningParametersIMpt();;
	...
	private float relationExclusionRatio = 0.035f;
...
//		List<Set<String>> startAndEndActivities = XLogManager.addStartAndEndActivities(log);
		...
		eventStream = LogStreamer.logStreamer(log, sb);
		...
		this.noiseFilterConfig = relationExclusionRatio == 0f ? NoiseFilterConfig.NoFilter : NoiseFilterConfig.Removal;
		this.relationExclusionRatio = this.noiseFilterConfig == NoiseFilterConfig.NoFilter ? 0f : (noiseFilterPercentage / 100.0f);
		...
		this.windowHeader = "P-value Diagram"+" (" + logFileName +  ", Initial window size = "+ initialwinSize + ", Window type = " + winConfig.toString() + ", Relation Type = " + RelationType + ", Relation noise filter percentage = " + relationExclusionRatio * 100 + "%" + ")";
...
//		List<Set<String>> startAndEndActivities = XLogManager.addStartAndEndActivities(log);
        ...
        this.relationExclusionRatio = 0f;
        this.withConflict = true;
        if(!isStandard)
        {
        	this.noiseFilterConfig = NoiseFilterConfig.Removal;
        	this.relationExclusionRatio = 3.5f / 100.0f;
		...
		if(this.driftConfig == DriftConfig.AlphaRelation)
			...
			RelationType = this.driftConfig.toString();
		this.windowHeader = "P-value Diagram"+" (Initial window size = "+ initialwinSize + ", Window type = " + winConfig.toString() + ", Relation Type = " + RelationType + ")";
	...
	public ControlFlowDriftDetector_EventStream(XLog logFIle, String logFileName, int winsize, DriftConfig Dcfg, 
			...
			NoiseFilterConfig noiseFilterConfig, Path logPath, float relationExclusionRatio,
...
//		miningParameters = new MiningParametersIMpt();
//		if(IMConfig == InductiveMinerConfig.IM)
//		{
//			// inductive miner with complete traces
...
//		}else if(IMConfig == InductiveMinerConfig.IMcpt)
//		{
//			// inductive miner with incomplete log of partial traces
//			miningParameters = new MiningParametersIMcpt();
//		}
		
		this.FEConfig = FEConfig;
		this.noiseFilterConfig = noiseFilterConfig;
		if(this.noiseFilterConfig == NoiseFilterConfig.NoFilter)
			relationExclusionRatio = 0;
		log = logFIle;
...
//		List<Set<String>> startAndEndActivities = XLogManager.getStartAndEndActivities(log);
		...
		this.windowHeader = "P-value Diagram"+" (" + logFileName +  ", Initial window size = "+ initialwinSize + ", Window type = " + winConfig.toString() + ", Relation Type = " + driftConfig + ", Relation noise filter threshhold = " + relationExclusionRatio * 100 + "%" + relationStrength + ")";
		this.relationExclusionRatio = relationExclusionRatio;
		this.withConflict = withConflict;
...
//						System.out.print("before: " + Follow_FreqMatrix[0].length + "  ");
						...
						if (this.noiseFilterConfig == NoiseFilterConfig.Removal)
							Follow_FreqMatrix = removeNoise(Follow_FreqMatrix, totalRelationFreq_bothWins[0], totalRelationFreq_bothWins[1]);
...
//						System.out.print("before: " + DC_FreqMatrix[0].length + "  ");
						...
						if (this.noiseFilterConfig == NoiseFilterConfig.Removal)
							DC_FreqMatrix = removeNoise(DC_FreqMatrix, totalRelationFreq_bothWins[0], totalRelationFreq_bothWins[1]);
						else if (this.noiseFilterConfig == NoiseFilterConfig.Aggregation)
...
//							pValue = chi.chiSquareTestDataSetsComparison(DC_FreqMatrix[1], DC_FreqMatrix[0]);
...
//							{
//								
//								for (int i = 0; i < DC_FreqMatrix.length; i++) {
//									for (int j = 0; j < DC_FreqMatrix[i].length; j++) {
//										System.out.print(DC_FreqMatrix[i][j] + ", ");
//									}
//									System.out.println();
//								}
//								
...
//		ArrayList<inLong> timePerformance = new ArrayList<>();
		...
		alphaRelations_det = new BasicLogRelationsExt(detectionWindowSubLog, summary);
		...
		alphaRelations_ref = new BasicLogRelationsExt(referenceWindowSubLog, summary);
...
//			System.out.println("before: " + Alpha_FreqMatrix2[0].length);
			...
			if (this.noiseFilterConfig == NoiseFilterConfig.Removal)
				Alpha_FreqMatrix2 = removeNoise(Alpha_FreqMatrix2, totalRelationFreq_bothWins[0], totalRelationFreq_bothWins[1]);
			else if (this.noiseFilterConfig == NoiseFilterConfig.Aggregation)
...
//					System.out.println(eventIndex + "," + winSize + "," + DC_FreqMatrix[1].length);
//						System.out.println("after: " + Alpha_FreqMatrix[0].length);
...
//						}
						...
						if(!Main.isLogGenerationForNick && charConfig == CharacterizationConfig.LogisticRegression)
							...
							charStatements = characterizer.characterizeDrift_KSpermutationTest(Relation_repFreq_map, RelationFreqMap_Queue, outputQueue,
									subLogB, subLogA, startActivities, endActivities/*, miningParameters*/);
//						else
//							return null;
...
//							}
			...
			eventIndex++;
			if(eventIndex < eventStream.size() /*&& eventIndex < 40000*/)
			{
		...
		if(pairRelation.getValue() != null)
		{
			...
			switch(pairRelation.getValue())
			{
				...
				case Length_Two_Loop_bi:
				{
					...
					Integer loopLengthTwoFreq_pair = alphaRelations.getLengthTwoLoops().get(pair);
					Integer loopLengthTwoFreq_opposed = alphaRelations.getLengthTwoLoops().get(opposed);
				...
				case Length_Two_Loop_ABA:
				{
					...
					Integer loopLengthTwoFreq_pair = alphaRelations.getLengthTwoLoops().get(pair);
					Integer causalFreq_opposed = alphaRelations.getDirectFollowsDependencies().get(opposed);
				...
				case Length_Two_Loop_BAB:
				{
					...
					Integer causalFreq_pair = alphaRelations.getDirectFollowsDependencies().get(pair);
					Integer loopLengthTwoFreq_opposed = alphaRelations.getLengthTwoLoops().get(opposed);
				...
				case Length_One_Loop:
				{
					...
					Integer loopLengthOneFreq_pair = alphaRelations.getLengthOneLoops().get(pair.getFirst());
				...
				case CONCURRENCY:
				{
					...
					Integer freq1 = alphaRelations.getDirectFollowsDependencies().get(pair);
					Integer freq2 = alphaRelations.getDirectFollowsDependencies().get(opposed);
...
//		Utils.writeToFile("countNonDoubleZero.csv", listOfLists);
	...
	public long [][] build_AlphaRelations_FrequencyMatrix(XLog subLog_detWin, XLog subLog_refWin, 
			...
			LogRelations alphaRelations_det, LogRelations alphaRelations_ref)
	{
...
//		System.out.println(distinctRelations.size());
		...
		Map<String, Integer> RelationFreqMap = new HashMap<>(); // for characterization
...
//			if(Math.abs(relationsFreqMatrixTemp[0][j] - relationsFreqMatrixTemp[1][j]) > 100)
//			{
//				
...
//		System.out.println("relationsFreqMatrix: " + (System.currentTimeMillis() - t1));
...
//						freq = rf.getRefWinFreq();
			...
			if(withCharacterization && (eventIndex % charWindowMoveInterval) == 0 && !isNewDriftDetected && pr != null && pr.getBehaviourRelation() != BehaviorRelation.Independence)
			{
		...
		if(withCharacterization && (eventIndex % charWindowMoveInterval) == 0 && !isNewDriftDetected)
		{
...
//				System.out.println();
	...
	public Map<String, Integer> getAlphaRelationsWithFrequency(XLog log, HashMap<String, Integer> relation_freq, 
			...
			Map<Pair<XEventClass, XEventClass>, RelationFrequency> pair_Relation_Map, boolean isDet, LogRelations alphaRelations)
	{
...
//		System.out.println("createLogInfo: " + (System.currentTimeMillis() - t1));
		t1 = System.currentTimeMillis();
		alphaRelations = new BasicLogRelationsExt(log, summary);
...
//		}
	...
	public void getAlphaRelationFrequency(XLog subLog, Map<String, Integer> relationsAndfrequncies, LogRelations alphaRelations, 
...
//		b_DF_a_freq = b_DF_a_freq != null ? b_DF_a_freq : 0;
		...
		Integer loopLengthOneFreq = alphaRelations.getLengthOneLoops().get(a);
		...
		}else
		{
			...
			Integer loopLengthTwoFreq = alphaRelations.getLengthTwoLoops().get(new Pair<XEventClass, XEventClass>(a, b));
			if(loopLengthTwoFreq != null && loopLengthTwoFreq >= 0)
			{
				...
				addPairRelationEntity(pair_Relation_Map, new Pair<XEventClass, XEventClass>(a, b), BehaviorRelation.Length_Two_Loop, loopLengthTwoFreq, isDet);
				
				// Find the relation of the opposite (b,a)
				loopLengthTwoFreq = alphaRelations.getLengthTwoLoops().get(new Pair<XEventClass, XEventClass>(b, a));
				if(loopLengthTwoFreq != null && loopLengthTwoFreq >= 0)
				{
					...
					addPairRelationEntity(pair_Relation_Map, new Pair<XEventClass, XEventClass>(b, a), BehaviorRelation.Length_Two_Loop, loopLengthTwoFreq, isDet);
					
				}else
				{
				
					Integer freq = alphaRelations.getDirectFollowsDependencies().get(new Pair<XEventClass, XEventClass>(b, a));
					if(freq != null && freq >= 0)
					{
						// Causal relation b -> a
			...
			}else 
			{
				...
				if(loopLengthTwoFreq != null && loopLengthTwoFreq >= 0)
				{
					...
					// Causal relation a -> b
					Integer freq = alphaRelations.getDirectFollowsDependencies().get(new Pair<XEventClass, XEventClass>(a, b));
					if(freq != null && freq >= 0)
					{
						// Causal relation a -> b
				...
				}else
				{
					...
					if(relationStrength != null && relationStrength >= 0)
					{
						...
						Integer freq1 = alphaRelations.getDirectFollowsDependencies().get(new Pair<XEventClass, XEventClass>(a, b));
						Integer freq2 = alphaRelations.getDirectFollowsDependencies().get(new Pair<XEventClass, XEventClass>(b, a));
					...
					}else
					{
						...
						if(freq != null && freq >= 0)
						{
							...
							freq = alphaRelations.getDirectFollowsDependencies().get(new Pair<XEventClass, XEventClass>(b, a));
							if(freq != null && freq >= 0)
							{
								// Causal relation b -> a
	...
	public void getAlphaRelationFrequency_withStrength(XLog subLog, Map<String, Integer> relationsAndfrequncies, LogRelations alphaRelations, 
			HashMap<String, Integer> relation_freq, HashMap<String, Integer> DF_relation_freq, HashMap<String, Integer> Activity_freq,	HashMap<String, Set<String>> Activity_TraceSet, 
	...
	*/
	...
	public long[][] aggregateNoise(long [][]freqMatrix, Integer totalRelationFreq_detWin, Integer totalRelationFreq_refWin)
	{
		...
		for (int i = 0; i < freqMatrix[1].length; i++) {
			...
			if (!fulfillsFrequencyRequirement(totalRelationFreq_detWin.intValue(), freqMatrix[0][i], relationExclusionRatio) &&
					!fulfillsFrequencyRequirement(totalRelationFreq_refWin.intValue(), freqMatrix[1][i], relationExclusionRatio))
			{
	...
	public long[][] removeNoise(long [][]freqMatrix, Integer totalRelationFreq_detWin, Integer totalRelationFreq_refWin)
	{
...
//		}
	...
	public int checkQSquareFrequencyRequirment(long runsFreqWindow[])
	{
		...
		int excludedColumnsCount = (int)Math.floor(((float)runsFreqWindow.length) * relationExclusionRatio);
