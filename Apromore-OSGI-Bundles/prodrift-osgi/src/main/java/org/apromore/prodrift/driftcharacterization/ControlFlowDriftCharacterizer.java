...
//	private Rengine engineR;
	...
	private int topCharzedDrifts = 1000;
	private int cutTopRelationsPercentage = 95;
...
//	    System.out.println();
		...
		if(!Main.isStandAlone) {
			try {
				...
				for (int i = 0; i < charzedDrifts.size(); ) {
					Entry<String, Map<String, XEventClass>> charzedDrift = charzedDrifts.get(i);
					fw.write(++i + "- " + charzedDrift.getKey() + ", , time," + (t2 - t1) + "\n");
				}
				fw.write("\n");
...
//	}
//
//	// Conditional OperatorChange: [-: 1)v->w(ABS), 2)w->x(ABS), 3)u->y(REL), 4)y->z(REL)] [+: 5)v->x(ABS), 6)u->w(ABS), 7)w->z(ABS)]
...
//	}
//
//	// Conditional OperatorChange Inverse: [-: 5)v->x(ABS), 6)u->w(ABS), 7)w->z(ABS)] [+: 1)v->w(ABS), 2)w->x(ABS), 3)u->y(REL), 4)y->z(REL)]
...
//	}
//
//	// Parallel OperatorChange: [-: 1)v->w(ABS), 2)w->x(ABS), 3)u->y(REL), 4)y->z(REL)] [+: 5)v->x(ABS), 6)u->w(ABS), 7)w->z(ABS), 8)w||y(ABS)]
...
//	}
//
//	// Parallel OperatorChange Inverse: [-: 5)v->x(ABS), 6)u->w(ABS), 7)w->z(ABS), 8)w||y(ABS)] [+: 1)v->w(ABS), 2)w->x(ABS), 3)u->y(REL), 4)y->z(REL)]
...
//	}
//
//	// Serial OperatorChange: [-: 1)w->x(ABS), 2)x->y(ABS), 3)t->v(ABS)] [+: 4)w->y(ABS), 5)t->x(ABS), 6)x->v(ABS)]
...
//	}
//
//	// Synchronize RemoveNoise_sum: [-: 4)z->y(ABS), 5)y->u(ABSorREL), 6)w->z(ABSorREL)] [+: 1)y||z(ABS), 2)w->y(ABSorREL), 3)z->u(ABSorREL)]
...
//		System.out.println(numOfPermutations);
		...
		List<Entry<PairRelation, RelationImpact>> permutation = new ArrayList<Map.Entry<PairRelation,RelationImpact>>();
		...
		for (int j = 0; j < numOfRelationsLists && isThereNewPerm; j++)
		{
			...
			}else
			{
				...
				if(j == numOfRelationsLists - 1)
				{
					...
					List<Entry<PairRelation, RelationImpact>> permutation2 = new ArrayList<Map.Entry<PairRelation,RelationImpact>>();
					for(Map.Entry<PairRelation,RelationImpact> entry : permutation) permutation2.add(entry);
...
//					}
	...
	private static Map<PairRelation, RelationImpact> sortByComparator_byCoefficient(Map<PairRelation, RelationImpact> unsortMap) {
		...
		// Convert Map to List
		List<Map.Entry<PairRelation, RelationImpact>> list =
				new LinkedList<Map.Entry<PairRelation, RelationImpact>>(unsortMap.entrySet());

		// Sort list with comparator, to compare the Map values
		Collections.sort(list, new Comparator<Map.Entry<PairRelation, RelationImpact>>() {
			public int compare(Map.Entry<PairRelation, RelationImpact> o1,
							   Map.Entry<PairRelation, RelationImpact> o2) {
		...
		for (Iterator<Map.Entry<PairRelation, RelationImpact>> it = list.iterator(); it.hasNext();) {
			Map.Entry<PairRelation, RelationImpact> entry = it.next();
	...
	private static Map<List<Entry<PairRelation, RelationImpact>>, InformationGain> sortByComparator_byDCG(Map<List<Entry<PairRelation, RelationImpact>>, InformationGain> unsortMap) {
		...
		// Convert Map to List
		List<Map.Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>> list =
				new LinkedList<Map.Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>>(unsortMap.entrySet());

		// Sort list with comparator, to compare the Map values
		Collections.sort(list, new Comparator<Map.Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>>() {
			public int compare(Map.Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain> o1,
							   Map.Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain> o2) {
		...
		for (Iterator<Map.Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>> it = list.iterator(); it.hasNext();) {
			Map.Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain> entry = it.next();
			sortedMap.put(entry.getKey(), entry.getValue());
		}
		return sortedMap;
	}
	...
	private static Multimap<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>> sortByComparator_byDCG3(Multimap<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>> unsortMap) {
		...
		// Convert Map to List
		List<Map.Entry<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>>> list =
				new LinkedList<Map.Entry<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>>>(unsortMap.entries());

		// Sort list with comparator, to compare the Map values
		Collections.sort(list, new Comparator<Map.Entry<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>>>() {
			public int compare(Map.Entry<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>> o1,
							   Map.Entry<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>> o2) {
		...
		for (Iterator<Map.Entry<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>>> it = list.iterator(); it.hasNext();) {
			Map.Entry<String, Entry<List<Entry<PairRelation, RelationImpact>>, InformationGain>> entry = it.next();
			sortedMap.put(entry.getKey(), entry.getValue());
		}
...
//		relPatsList.add(new RelationChange("y", "z", BehaviorRelation.Causal, FrequencyChange.Increase, FrequencyChangeType.REL, 1f));
	...
	public static void characterizeDrifts_TreeComparison(
			Path logParentPath,
			String logName,
			XLog subLogB,
			XLog subLogA,
