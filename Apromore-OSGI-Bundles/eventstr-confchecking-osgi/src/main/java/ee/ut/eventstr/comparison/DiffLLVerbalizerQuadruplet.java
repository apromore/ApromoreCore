 ...
 * @date 23/11/2016
 */
public class DiffLLVerbalizerQuadruplet<T> {
...
//	private Set<String> statements;
	private HashSet<Quadruplet<String, Set<XTrace>, Set<XTrace>, Integer>> differences;

	private Table<BitSet, BitSet, Map<Integer, int[]>> globalDiffs;

	public DiffLLVerbalizerQuadruplet(PESSemantics<T> pes1, String logName1, PESSemantics<T> pes2, String logName2) {
		this.pes1 = pes1;
		this.pes2 = pes2;
...
//		this.statements = new HashSet<String>();
	...
	public void addPSP(List<Operation> opSeq, int cost) {
		opSeqs.add(opSeq);
		costs.add(cost);
	}

	public Set<Quadruplet<String, Set<XTrace>, Set<XTrace>, Integer>> verbalize() {
		...
		for (int index = 0; index < opSeqs.size(); index++) {
			...
			verbalizeDifferences(opSeq, diffIndexesList, index, costs.get(index));
		}
		
		return differences;
	}
	
	private void verbalizeDifferences(List<Operation> opSeq, List<int[]> diffIndexesList, int index, int cost) {
		int[] diffIndexes;
...
//                                getCorrectContext(firstMatching.getLabel()), firstHiding.getLabel(), secondHiding.getLabel());
                        ...
                        addDifference(new Quadruplet(statement, pes1.getTracesOf((Integer)firstHiding.target), pes2.getTracesOf((Integer)secondHiding.target), cost));
					}
				} 
				else {
...
//                                    translate(past, opSeq), translate(interval, opSeq));
							...
							addDifference(new Quadruplet(statement, getTraces(interval, opSeq, Op.LHIDE), getTraces(past, opSeq, Op.MATCH), cost));
						}
					} 
					else {
...
//														translate(context1, 1));
										...
										addDifference(new Quadruplet<>(statement,getTracesOfBS(context1, 1), new HashSet<>(), cost));
									}
								}
								else {
									context2.or(dconflict);
...
//																translate(past, opSeq));
												...
												addDifference(new Quadruplet<>(statement, getTracesOfBS(interval, opSeq, 1), getTracesOfBS(past, opSeq, 2), cost));
											}
										}
									}
								}
...
//											translate(past, opSeq), translate(interval, opSeq));
							...
							addDifference(new Quadruplet(statement, getTraces(past, opSeq, Op.MATCH), getTraces(interval, opSeq, Op.RHIDE), cost));
						}
					} 
					else {
...
//														translate(context2));
										...
										addDifference(new Quadruplet(statement, getTraces(context2, opSeq, Op.LHIDE), getTraces(context2, opSeq, Op.RHIDE), cost));
									}
								} 
								else {
									context1.set(e1p);
...
//															translate(interval, opSeq), translate(past, opSeq));
											...
											addDifference(new Quadruplet(statement,  getTracesOfBS(past, opSeq,1), getTracesOfBS(interval, opSeq,2), cost));
										}
									}
								}
							}
...
//				}
	...
	private void addDifference(Quadruplet<String, Set<XTrace>, Set<XTrace>, Integer> quadruplet) {
		boolean found = false;
		for(Quadruplet<String, Set<XTrace>, Set<XTrace>, Integer> old : differences) {
			if(old.equals(quadruplet)) {
				old.getB().addAll(quadruplet.getB());
				old.getC().addAll(quadruplet.getC());
				found = true;
			}
		}
		if(!found) {
			differences.add(quadruplet);
		}
	}
...
//		String statement = String.format("In the normal behavior, %s %s %s, while in the deviant behavior %s %s %s", e1l, br1, e1pl, e2l, br2, e2pl);
		...
		addDifference(new Quadruplet(statement, intersectionP1, intersectionP2, 0));
	}
