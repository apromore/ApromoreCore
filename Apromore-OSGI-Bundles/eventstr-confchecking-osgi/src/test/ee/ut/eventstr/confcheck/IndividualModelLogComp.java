...
public class IndividualModelLogComp {
	...
	private final String basefolder = "models/samplePNMLna/";
	...
	public void test() throws Exception {
		String pnmlfolder = basefolder;
		String logfiletemplate = pnmlfolder + "%s.xes";
		
		String filebase = "sampleAtomic";
...
//		runTest(pnmlfolder, logfiletemplate, filebase + pnml + ".pnml",  filebase + log + "_log");
		
		testAllModels(pnmlfolder, logfiletemplate, filebase);
	}
	
	public void testAllModels(String pnmlfolder, String logfiletemplate, String filebase) throws Exception {
		String pnml = "Base";
		String [] logs = {"E1", "E2", "E3", "C1", "C2", "C3", "C4", "C5", "B1", "B2", "B2_2"};
		
		for (int i = 0; i < logs.length; i++) {
			runTest(pnmlfolder, logfiletemplate, filebase + pnml + ".pnml", filebase + logs[i] + "_log");
		}
	}
	...
	public NewUnfoldingPESSemantics<Integer> getUnfoldingPESExample(String filename, String folder) throws JDOMException, IOException {
		...
		for (Transition t: net.getTransitions()) {
			if (!(t.getName().equals(""))) { // get rid of silent transitions
