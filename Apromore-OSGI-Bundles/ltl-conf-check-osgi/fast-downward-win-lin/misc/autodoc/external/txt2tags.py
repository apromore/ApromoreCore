...
ACTIONS  = {'help'       :0 , 'version'    :0 , 'gui'        :0 ,
            ...
            'dump-source':0 , 'list-targets':0}
MACROS   = {'date' : '%Y%m%d',  'infile': '%f',
            'mtime': '%Y%m%d', 'outfile': '%f'}
SETTINGS = {}         # for future use
NO_TARGET = ['help', 'version', 'gui', 'toc-only', 'dump-config', 'dump-source','list-targets']
...
_("      --list-targets  print a list of all the available targets and exit"),
'',
_("Turn OFF options:"),
...
def aa_header(header_data):
	header = aa_line(AA_CHARS['bar2']) + LB + LB
	for h in 'HEADER1', 'HEADER2', 'HEADER3':
		if header_data[h]:
			header += aa_box(header_data[h]) + LB + LB
...
class CommandLine:
	...
	def _compose_long_opts(self):
		...
		ret.extend(['no-list-targets'])                      # turn OFF
	...
	def get_raw_config(self, cmdline=[], ignore=[], filter_=[], relative=0):
		...
		for name,value in temp:
			if (not filter_ and not ignore) or \
			   (filter_ and name in filter_) or \
	...
	def sanity(self, config, gui=0):
		...
		if not gui:
			...
			if not target:
				...
				_("Run 'txt2tags --list-targets' to see all the available targets."))
...
def doFooter(config):
	...
	if not rules.get('blanksaround'+BLOCK.last): ret.append('')
...
def exec_command_line(user_cmdline=[]):
	global CMDLINE_RAW, RC_RAW, DEBUG, VERBOSE, QUIET, GUI, AA_CHARS, AA_LCHARS, Error
	...
	if cmdline_parsed.get('list-targets'):
		listTargets()
		Quit()

	if 'ascii-art' in cmdline_parsed:
