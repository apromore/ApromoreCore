...
public class SAPscalPerfTest2 {
	...
	public void test() throws Exception {
		...
		slowfiles.add("1Ku_9bjf");
		slowfiles.add("1Tr_gcc2");
		slowfiles.add("1Un_j7jq");
//		slowfiles.add("1Ve_n0z8");
		
		Set<String> emptyfiles = new HashSet<String>();
		emptyfiles.add("1Er_ixso_1");
		
		Set<String> retryfiles = new HashSet<String>();
//		retryfiles.add("1Ex_dy43_1");
		retryfiles.add("1Ve_n0z8");
//		retryfiles.add("1An_kg4m");
		...
		limitSize(logfiletemplate, files, 2048);
		
//		runAllTestsInFolder(pnmlfolder, logfiletemplate, files, "summary_new.txt");
		runAllTestsInFolder(pnmlfolder, logfiletemplate, retryfiles, "summary_slow.txt");
		
//		analyzeDifferences("1Ve_n0z8_log_noise05", logfiletemplate, "1Ve_n0z8.pnml", pnmlfolder);
	}
	
	public void limitSize(String logfileteplate, Set<String> files, int maxsize) {
		Set<String> largefiles = new HashSet<String>();
		
		for (String f: files) {
			if (new File(String.format(logfileteplate, f + "_log")).length() / 1024 > maxsize) {
				largefiles.add(f);
			}
		}
		files.removeAll(largefiles);
	}
