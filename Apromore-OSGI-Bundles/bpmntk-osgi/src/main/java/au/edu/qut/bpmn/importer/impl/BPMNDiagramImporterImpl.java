package au.edu.qut.bpmn.importer.impl;
/*
...
//            File schemaFile = new File(classLoader.getResource("./xsd/BPMN20.xsd").getFile());
            File schemaFile = new File(this.getClass().getResource("/xsd/BPMN20.xsd").toURI());
            ...
            for( JAXBElement<? extends TRootElement> process : processes )
                if (process.getValue() instanceof TProcess) {
                    System.out.println("Analyzing Process: " + process.getValue().getId());
                    unfoldProcess(process.getValue(), null);
                    System.out.println("Added Process: " + process.getValue().getId());
                }
            ...
            System.out.println("importBPMNDiagram: unable to unmarshall the xml file.");
            ue.printStackTrace();
            throw ue;
        } catch( JAXBException je ) {
            System.out.println("importBPMNDiagram: unable to create the JAXBContext.");
            je.printStackTrace();
            throw je;
        } catch(Exception e) {
            System.out.println("importBPMNDiagram: error creating the importBPMNDiagram.");
            e.printStackTrace();
            throw e;
        }
    }
    ...
    private void handleCollaboration(TCollaboration collaboration) {
        ...
        for( TParticipant participant : collaboration.getParticipant() ) {
            ...
            if( participant.getProcessRef() != null ) {
                ...
                System.out.println("Added Pool: " + participant.getId() + " = " + s.getId());
            } else System.out.println("Unaddable Pool: " + participant.getId());
        }
    ...
    private void buildEdges() {
        ...
        for( TSequenceFlow flow : flows ) {
            ...
            if( idToNode.containsKey(src) && idToNode.containsKey(tgt) ) {
                ...
                //System.out.println("Adding Event Flow: " + src + " > " + tgt);
            } else System.out.println("[" + idToNode.containsKey(src) + ":" + idToNode.containsKey(tgt)  + "] Unfixable Flow: " + src + " > " + tgt);
        }
        ...
        for( TAssociation association : associations ) {
            ...
            if( idToNode.containsKey(src) && idToNode.containsKey(tgt) ) {
                ...
                //System.out.println("Added Association(" + a.getDirection() + "): " + src + " > " + tgt);
            } else System.out.println("[" + idToNode.containsKey(src) + ":" + idToNode.containsKey(tgt)  + "] Unfixable Association: " + src + " > " + tgt);
        }
            ...
            for( JAXBElement je : dataAssociation.getSourceRef() )
                if( je.getValue() instanceof TBaseElement ) {
                    ...
                    if( idToNode.containsKey(src) && idToNode.containsKey(tgt) ) {
                        ...
                        //System.out.println("Added DataAssociation: " + src + " > " + tgt);
                    } else System.out.println("[" + idToNode.containsKey(src) + ":" + idToNode.containsKey(tgt)  + "] Unfixable DataAssociation: " + src + " > " + tgt);
                }
        }
        ...
        for( TMessageFlow messageFlow : messageFlows ) {
            ...
            if( idToNode.containsKey(src) && idToNode.containsKey(tgt) ) {
                ...
                //System.out.println("Added MessageFlow: " + src + " > " + tgt);
            } else System.out.println("[" + idToNode.containsKey(src) + ":" + idToNode.containsKey(tgt)  + "] Unfixable MessageFlow: " + src + " > " + tgt);
        }
    ...
    private void fixBounds() {
        for( Event e : boundToFix.keySet() )
            ...
            } else System.out.println("Unfixable boundaryEvent: " + e.getLabel() + " > " + boundToFix.get(e));
        ...
        for( Event e : diagram.getEvents() )
            if( (e.getEventUse() == Event.EventUse.CATCH) && (e.getEventType() == Event.EventType.INTERMEDIATE) && (diagram.getInEdges(e).size() == 0) ) {
                if( e.getParentSubProcess() != null ) {
                    ...
                    System.out.println("found ghost boundary event: " + e.getId() + " for " + e.getParentSubProcess().getId());
                ...
                } else System.out.println("error fixing ghost boundary event: " + e.getId());
            }
    }
    ...
    private void checkCollapsedPools() {
        ...
        for( Swimlane p : diagram.getPools() ) {
            ...
            if(uselessPool) {
                ...
                System.out.println("removed useless pool: " + p.getId());
            }
        }
    }
    ...
    private void addDataObject(TDataObject dataObject, SubProcess parentProcess, Swimlane pool) {
        ...
        //System.out.println( (parentProcess==null ? "null" : parentProcess.getId()) + " - " + (pool==null ? "null" : pool.getId()) + " - added dataObject: " + dataObject.getId() + " = " + d.getId());
    }
    ...
    private void addTextAnnotation(TTextAnnotation textAnnotation, SubProcess parentProcess, Swimlane pool) {
        ...
        //System.out.println( (parentProcess==null ? "null" : parentProcess.getId()) + " - " + (pool==null ? "null" : pool.getId()) + " - added textAnnotation:" + textAnnotation.getId() + " = " + ta.getId());
    }
    ...
    private void addEvent(TEvent event, SubProcess parentProcess, Swimlane pool) {
        ...
        if( event instanceof TCatchEvent ) {
            ...
            if( event instanceof TStartEvent ) type = Event.EventType.START;
                ...
                //System.out.println("Boundary event found: " + activityID + " < " + event.getId() );
            }
        ...
        //System.out.println("Added Event(" + e.getEventType() + "," + e.getEventTrigger() + "): " + event.getId() + " = " + e.getId());
    }
    ...
    private void addTask(TTask task, SubProcess parentProcess, Swimlane pool) {
        ...
        //System.out.println("(" + isBCompensation + ") Added Activity: " + task.getId() + " = " + a.getId());
    }
    ...
    private void addGateway(TGateway gateway, SubProcess parentProcess, Swimlane pool) {
        ...
        //System.out.println("Added gateway: " + gateway.getId() + " = " + g.getId());
    }
    ...
    private void addCallActivity(TCallActivity callActivity, SubProcess parentProcess, Swimlane pool) {
        ...
        //System.out.println("(" + isBCompensation + ") Added CallActivity: " + callActivity.getId() + " = " + ca.getId());
    }
    ...
    private void addSubProcess(TSubProcess subProcess, SubProcess parentProcess, Swimlane pool) {
        ...
        System.out.println("Analyzing subProcess: " + subProcess.getId());
        unfoldProcess(subProcess, sp);
        System.out.println("Added SubProcess: " + subProcess.getId() + " = " + sp.getId());
    }
    ...
    private void unfoldProcess(TBaseElement root, SubProcess parentProcess) {
        ...
        if( root instanceof TProcess ) {
            ...
            if( (pool == null) && (processes.size() > 1) ) {
                ...
                System.out.println("Added Pool unknown: " + pool.getId());
            }
    ...
    private void addLanes(TLaneSet laneSet, Swimlane parent) {
        ...
        for( TLane lane : laneSet.getLane() ) {
            ...
            for( JAXBElement node : lane.getFlowNodeRef() ) {
                ...
                else System.out.println("ERROR setting a lane for a node that is instanceof: " + node.getValue().getClass().getName());
                //System.out.println("Trying to set Lane > " + lane.getId() + " for:  " + id);
                if( idToNode.containsKey(id) ) {
                    idToNode.get(id).setParentSwimlane(s);
                    //System.out.println("Lane set.");
                } else System.out.println("Unsettable lane for: " + id);
            }
    ...
    private class BPMNDiagramImpr extends BPMNDiagramImpl {
        ...
        public void removeEdge(DirectedGraphEdge edge) {
            ...
            try {
                ...
                System.out.println("NullPointerException removing the edge (" + edge.getClass().getSimpleName() + "): " + edge.getLabel());
            }
        }
        ...
        public void removeNode(DirectedGraphNode node) {
            ...
            try {
                ...
                System.out.println("NullPointerException removing the node (" + node.getClass().getSimpleName() + "): " + node.getLabel());
            }
        }
    }
