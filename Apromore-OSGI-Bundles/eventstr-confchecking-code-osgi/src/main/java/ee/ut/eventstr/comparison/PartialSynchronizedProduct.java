...
public class PartialSynchronizedProduct<T> {
	...
	public PartialSynchronizedProduct<T> perform() {
		...
		root = getState(new BitSet(), HashMultiset.<String> create(), new BitSet()).getSecond();
		open.offer(root);
...
//			System.out.print(".");
	...
	private Pair<StateHint,State> getState(BitSet c1, Multiset<String> labels, BitSet c2) {
		...
		if (stateSpaceTable.contains(c1, c2)) {
			...
			if (newState.weight == pivot.weight) {
				...
				for (State sibling: mergedStates.get(pivot))
					if (sibling.labels.containsAll(labels)) {
						found = true;
						break;
					}
				if (!found) {
		...
		for (State s: lstates) {
			...
			for (Operation op: descendants.get(s)) {
				if (relevantStates == null || relevantStates.contains(op.nextState)) {
					...
					if (op.op == Op.MATCH)
						out.printf("\tn%d -> n%d [label=\"match(%s)\"];\n", src, tgt, op.label);
					else if (op.op == Op.LHIDE)
						out.printf("\tn%d -> n%d [label=\"lhide(%s)\"];\n", src, tgt, op.label);
					else
						out.printf("\tn%d -> n%d [label=\"rhide(%s)\"];\n", src, tgt, op.label);
				}
			}
		}
		out.println("}");
...
//			System.out.printf("Earliest missmatch: %s, %s\n", earliestMissmatch, earliestMissmatch.nextState);
			if (earliestMissmatch != null) {
				...
				if (labelOperationMap.get(earliestMissmatch.label).size() > 1) {
					...
					PriorityQueue<Operation> leftHidings = new PriorityQueue<Operation>(11,new Comparator<Operation>() {
						public int compare(Operation o1, Operation o2) {
							return Float.compare(o1.nextState.weight, o2.nextState.weight);
						}
					});
					PriorityQueue<Operation> rightHidings = new PriorityQueue<Operation>(11, new Comparator<Operation>() {
						public int compare(Operation o1, Operation o2) {
							return Float.compare(o1.nextState.weight, o2.nextState.weight);
						}
					});					
					for (Operation o: labelOperationMap.get(eventLabel)) {
						if (earliestMissmatch != o) {
							if (earliestMissmatch.op == Op.LHIDE && o.op == Op.RHIDE) {
								...
								return String.format("In model 1 the task \"%s(%s)\" occurs just after tasks \"%s\", whereas in model 2 the same task occurs only after \"%s\"", earliestMissmatch.label, earliestMissmatch.target, str1, str2);
								...
								return String.format("In model 2 the task \"%s(%s)\" occurs just after tasks \"%s\", whereas in model 1 the same task occurs only after \"%s\"", earliestMissmatch.label, earliestMissmatch.target, str1, str2);
							}
							if (o.op == Op.RHIDE)
					...
					if (earliestMissmatch.op == Op.LHIDE) {
						...
						if (seq.size() > 0)
							return String.format("Task \"%s(%s)\" is repeated %d more times in model 1", earliestMissmatch.label, earliestMissmatch.target, seq.size());
					} else {
						List<Operation> seq = new ArrayList<>();
						Operation prev = earliestMissmatch;
						for (Operation o: leftHidings) {
						...
						if (seq.size() > 0)
							return String.format("Task \"%s(%s)\" is repeated %d more times in model 2", earliestMissmatch.label, earliestMissmatch.target, seq.size());						
					}
					
//					for (String eventLabel: labelOperationMap.keySet()) {
//						if (labelOperationMap.get(eventLabel).size() > 1) {
//							PriorityQueue<Operation> hidings = new PriorityQueue<>(new Comparator<Operation>() {
//								public int compare(Operation o1, Operation o2) {
//									return Float.compare(o1.nextState.weight, o2.nextState.weight);
//								}
//							});
//							List<Operation> matchings = new ArrayList<>();
//							for (Operation op: labelOperationMap.get(eventLabel)) {
//								if (op.op == Op.MATCH)
//									matchings.add(op);
//								else
//									hidings.add(op);
//							}
//							
//							if (matchings.size() > 0 && hidings.size() > 0) {
//								if (hidings.peek().op == Op.LHIDE)
//									return String.format("Task \"%s(%s)\" is repeated %d more times in model 1", earliestMissmatch.label, earliestMissmatch.target, hidings.size());
//								else
//									return String.format("Task \"%s(%s)\" is repeated %d more times more in model 2", earliestMissmatch.label, earliestMissmatch.target, hidings.size());
//							}
//						}
//					}
				...
				if (targetPair != null) {
//					System.out.printf("Complementary pair: %s\n", targetPair);
					if (targetPair.getFirst().intValue() == ((Integer)earliestMissmatch.target).intValue())
						return String.format("Task \"%s(%s)\" can be skipped in model 1, whereas in model 2 it is always executed", earliestMissmatch.label, earliestMissmatch.target);
					if (targetPair.getSecond().intValue() == ((Integer)earliestMissmatch.target).intValue())
						return String.format("Task \"%s(%s)\" can be skipped in model 2, whereas in model 1 it is always executed", earliestMissmatch.label, earliestMissmatch.target);
					...
					for (Pair<Integer, Integer> pair: localMatches) {
						...
						if (!br1.equals(br2)) {
							return String.format("Models exhibit differences in the behavior relation of the following tasks: %s[%s], %s[%s]: %s vs %s", targetPair, pes1.getLabel(targetPair.getFirst()), pair, pes1.getLabel(pair.getFirst()), br1, br2);
						}
					}
				} else {
					if (earliestMissmatch.op == Op.LHIDE) {
//						if (pes2.getLabels().contains(earliestMissmatch.label))
//							return String.format("Task \"%s(%s)\" can be skipped in model 1", earliestMissmatch.label, earliestMissmatch.target);
						...
						return String.format("Task \"%s(%s)\" occurs in model 1 just after tasks \"%s\", whereas the same task does not occur in model 2", earliestMissmatch.label, earliestMissmatch.target, str);					
					} else {
//						if (pes1.getLabels().contains(earliestMissmatch.label))
//							return String.format("Task \"%s(%s)\" can be skipped in model 2", earliestMissmatch.label, earliestMissmatch.target);
						...
						return String.format("Task \"%s(%s)\" occurs in model 2 just after tasks \"%s\", whereas the same task does not occur in model 1", earliestMissmatch.label, earliestMissmatch.target, str);
					}
				}
			}
			return null;
