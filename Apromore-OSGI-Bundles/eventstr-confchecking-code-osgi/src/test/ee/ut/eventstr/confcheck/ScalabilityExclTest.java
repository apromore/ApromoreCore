...
public class ScalabilityExclTest {
	String logfilename;
	String bpmnfilename;
	...
	public void test() throws Exception {
		logfilename ="concTestLog";
		bpmnfilename = "concTest";
		logfiletemplate	= "E:/JavaProjects/workspace/FancyJavaUtilities/logs/concurrency/%s.mxml";
		bpmnfolder = "E:/JavaProjects/workspace/FancyJavaUtilities/bpmn/concurrency/";
...
//		testAll();
	...
	public PrimeEventStructure<Integer> getLogPESExample(String logfilename, String logfiletemplate) throws Exception {
		...
		branchcount = Integer.parseInt(logfilename.substring(logfilename.length() - 2, logfilename.length() - 1));
		branchlength = Integer.parseInt(logfilename.substring(logfilename.length() - 1));
		
		Multimap<String, String> concurrency = HashMultimap.create();
		for (int branch1 = 0; branch1 < branchcount; branch1++) {
			for (int branch2 = 0; branch2 < branchcount; branch2++) {
				if (branch2 == branch1) {
					branch2++;
				}
				else {
					for (int act1 = 0; act1 < branchlength; act1++) {
						for (int act2 = 0; act2 < branchlength; act2++) {
							concurrency.put("A" + (branch1 * branchlength + act1 + 1), "A" + (branch2 * branchlength + act2 + 1));
						}
					}
				}
			}
