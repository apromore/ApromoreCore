...
public class SAPscalPerfTest3 {
	...
	public void test() throws Exception {
		String pnmlfolder = 
				"E:/Documents/NICTA/SAPlogs/sap_tauless2/"
				;
...
//		problemfiles.add("1Un_j7jq");
		...
		limitSize(logfiletemplate, files, 0, 2560);
//		limitSize(logfiletemplate, files, 2561, 4096);
		
//		runAllTestsInFolder(pnmlfolder, logfiletemplate, files, "summary_tauless2.txt");	
		
		
		//** use this:
		String pnmlfilename = "1An_kmy0.pnml";
		String logfilename = "1An_kmy0_log";
		debugTest(pnmlfolder, logfiletemplate, pnmlfilename, logfilename);
	...
	public long debugTest(String pnmlfolder, String logfiletemplate, String pnmlfilename, String logfilename) throws Exception {
	...
	public void runAllTestsInFolder(String pnmlfolder, String logfiletemplate, Set<String> files, String summaryfile) throws Exception {
		...
		for (String f: files) {
			...
			System.out.print(f + ": ");
			System.out.println(new File(String.format(logfiletemplate, logfilename)).length() / 1024 + " kb");
	...
	public NewUnfoldingPESSemantics<Integer> getUnfoldingPESExample(String filename, String folder) throws JDOMException, IOException {
		...
		for (Transition t: net.getTransitions()) {
			if ((!t.getName().startsWith("helper ")) && 
					(!t.getName().startsWith("xor/and")) &&
					(!t.getName().startsWith("start ")) &&
					(!t.getName().startsWith("0")) &&
					(!t.getName().startsWith("1")) &&
					(!t.getName().startsWith("2")) &&
					(!t.getName().startsWith("3")) &&
					(!t.getName().startsWith("4")) &&
					(!t.getName().startsWith("5")) &&
					(!t.getName().startsWith("6")) &&
					(!t.getName().startsWith("7")) &&
					(!t.getName().startsWith("8")) &&
					(!t.getName().startsWith("9"))
				) {
