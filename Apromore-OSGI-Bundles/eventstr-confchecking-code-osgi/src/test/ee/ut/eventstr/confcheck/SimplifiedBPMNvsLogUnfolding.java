...
import ee.ut.eventstr.comparison.NMOAOpenPartialSynchronizedProduct;
...
public class SimplifiedBPMNvsLogUnfolding {
...
//				"models/simple/"
		...
		NMOAOpenPartialSynchronizedProduct<Integer> psp = new NMOAOpenPartialSynchronizedProduct<Integer>(logpes, bpmnpes);
		psp.perform()
			.prune()
			;
...
//		int initial = 363, num = 4;
		...
		for (int i = initial; i < initial + num; i++) { XTrace trace = log.get(i);			
			PORun porun = new AbstractingShortLoopsPORun(alphaRelations, trace);
			runs.add(porun);
		}
